...
v5 adds the live retrieval-resolution API validator. The repository now checks
POST /v1/release and POST /v1/resolve as HTTP endpoints, not only script-level
engines.
"""
...
def run_command(name: str, cmd: Sequence[str]) -> Dict[str, Any]:
    ...
    return {"name": name, "cmd": list(cmd), "returncode": result.returncode, "ok": result.returncode == 0, "stdout": result.stdout, "stderr": result.stderr}
...
def copy_full_stack_artifacts(output_dir: Path) -> Dict[str, Any]:
    artifact_dir = output_dir / "full_stack_validator_v5"
    ...
    for extra in [
        ...
        "local_release_api_outputs",
        "local_resolution_api_outputs",
...
def run_full_stack_validator(output_dir: Path) -> Dict[str, Any]:
    if not FULL_STACK_VALIDATOR.exists():
        return {"name": "full_stack_validator_v5", "ok": False, "returncode": 127, "stdout": "", "stderr": f"Missing {FULL_STACK_VALIDATOR}", "summary": {"exists": False}, "artifacts": {"copied": []}}
    command = run_command("full_stack_validator_v5", [sys.executable, str(FULL_STACK_VALIDATOR)])
...
def write_proof_markdown(output_dir: Path, proof: Dict[str, Any]) -> None:
    lines = [
        "# CI Proof Pack v5", "", f"Status: **{'PASS' if proof['ok'] else 'FAIL'}**", f"Proof hash: `{proof['proof_hash']}`", "",
        "## Environment", "", f"- Python: `{proof['environment']['python']}`", f"- Platform: `{proof['environment']['platform']}`",
        f"- Commit: `{proof['git']['commit']}`", f"- Branch: `{proof['git']['branch']}`", "",
        "## Gates", "", f"- Compile package: `{'PASS' if proof['compile_ok'] else 'FAIL'}`",
    ]
    ...
    lines.append(f"- full_stack_validator_v5: `{'PASS' if full_stack.get('ok') else 'FAIL'}`")
    fs = full_stack.get("summary", {}) or {}
    if fs.get("exists"):
        lines.extend([
            "", "## Full-stack validator v5", "", f"- Overall OK: `{fs.get('ok')}`",
            ...
            f"- Release API: `{fs.get('release_api_pass')}` pass, `{fs.get('release_api_fail')}` fail",
            f"- Resolution API: `{fs.get('resolution_api_pass')}` pass, `{fs.get('resolution_api_fail')}` fail",
    ...
    for suite in proof["suites"]:
        lines.extend([f"### {suite['name']}", "", f"Cases: {suite['case_count']}", f"Passed: {suite['pass_count']}", f"Failed: {suite['fail_count']}", f"Summary hash: `{suite['summary_hash']}`", "", "| Case | OK | Action | Classification | Risk | Confidence | Route agreement | Reasons |", "|---|---:|---|---|---:|---:|---:|---|"])
        for result in suite["results"]:
            reasons = ", ".join(result.get("dominant_reasons", []))
            lines.append(f"| {result['case_id']} | {str(result['ok'])} | {result['actual_action']} | {result['actual_classification']} | {result['final_risk']:.3f} | {result['confidence']:.3f} | {result['route_agreement']:.3f} | {reasons} |")
        lines.append("")
    if not full_stack.get("ok"):
        lines.extend(["## Full-stack failure output", "", "### stdout", "", "```text", full_stack.get("stdout", ""), "```", "", "### stderr", "", "```text", full_stack.get("stderr", ""), "```"])
    (output_dir / "CI_PROOF_PACK_REPORT.md").write_text("\n".join(lines), encoding="utf-8")
...
def build_proof(output_dir: Path, benchmark_cases: Path) -> Dict[str, Any]:
    ...
    suites = [run_case_suite("smoke_default", None, output_dir), run_case_suite("benchmark_v1", benchmark_cases, output_dir)]
    full_stack = run_full_stack_validator(output_dir)
    proof_payload = {
        "proof_pack": "ci_proof_pack_v5",
        ...
        "git": {"commit": git_value(["rev-parse", "HEAD"]), "branch": git_value(["rev-parse", "--abbrev-ref", "HEAD"]), "status_porcelain": git_value(["status", "--porcelain"])},
    ...
    (output_dir / "ci_proof_pack_summary.json").write_text(json.dumps(proof_payload, indent=2, sort_keys=True, ensure_ascii=False), encoding="utf-8")
    write_proof_markdown(output_dir, proof_payload)
    return proof_payload


def main() -> None:
    parser = argparse.ArgumentParser(description="Run CI Proof Pack v5 for the AI trust stack")
    ...
    proof = build_proof(Path(args.out_dir), Path(args.benchmark_cases))
    print(json.dumps({"ok": proof["ok"], "out_dir": str(args.out_dir), "proof_hash": proof["proof_hash"]}, indent=2, sort_keys=True))
