...
\text{gate}\to
\text{fixed state}\to
\text{persistent record}\to
\text{finite-channel Hilbert extension}
\end{gathered}}
\label{eq:chain}
\end{equation}
A later object is never used to redefine an earlier one after a defect has been observed.  The Hilbert-space extension is placed after the finite record theorem because it strengthens the target-support layer without being needed to define the finite record itself.
...
T14 & Finite-channel Hilbert compression & Infinite ambient positivity is reduced to an $m\times m$ Hermitian certificate when the adverse information has $m$ channels. \\
T15 & Minimal target-repair dimension & The exact channel count is the target-relevant blind dimension. \\
T16 & Strict channel reserve & $\beta<1$ yields a quantitative source-relative positivity margin. \\
T17 & Projection-defect positivity & Orthogonal source completion leaves a positive Gram defect. \\
\bottomrule
\end{longtable}
